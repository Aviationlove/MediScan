...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function displayDetections(detections) {
    ...
    if (!detections || detections.length === 0) {
      detectionList.innerHTML =
        '<div class="detection-item"><div class="detection-content"><h4 class="text-gray-700 font-medium">No fractures detected</h4><p class="text-gray-600">The analysis did not identify any fractures in this image.</p></div></div>';
      return;
    }
    ...
    detections.forEach((detection, index) => {
      ...
      detectionItem.className =
        "detection-item mb-3 p-3 border-l-4 border-gray-300 bg-gray-50 rounded shadow-sm";
      detectionItem.innerHTML = `
                <div class="detection-content">
                    ...
                    <div class="location-info text-sm text-gray-600 mt-1">
                        <span>Location: x:${Math.round(
                          detection.box.x1
                        )}-${Math.round(detection.box.x2)}, y:${Math.round(
        detection.box.y1
      )}-${Math.round(detection.box.y2)}</span>
                    </div>
                </div>
            `;
  ...
  function checkServerStatus() {
    fetch(`${API_URL}/status`)
      ...
      .catch((error) => {
        ...
        alert(
          "Warning: Could not connect to detection server. Please make sure the backend server is running."
        );
      });
  }
});